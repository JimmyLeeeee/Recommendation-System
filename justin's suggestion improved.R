...
##########
###### arm_max_cdf: the function that input sample matrix and observed value to get its cdf prob.
...
##########
arm_max_cdf <- function(sample_matrix,value){
  
  samp_maxes <- rep(NA,ncol(sample_matrix))
  #select the maximum over a single draw from each arm. Repeat tau*T/K times
  for(i in 1:ncol(sample_matrix)){
    samp_maxes[i]<-max(sample_matrix[,i])
  }
  
  cdf_user <- ecdf(samp_maxes)
  value_prob <- cdf_user(value)
  return(value_prob)
}
...
ks_stat <- function(K=10,T=1000,min_mean=0,max_mean=10,min_sd=0,max_sd=1,
  ...
  user_prob_vector <- arm_max_cdf(user_sample_matrix,observed)
